Feature: Detección de finalización de partida por vaciado del tablero o agotamiento de movimientos
  ...
  Scenario Outline: Toda ejecución exitosa del orquestador decrementa exactamente 1 movimiento
    Given una sesión con movesRemaining = 5 y status = IN_PROGRESS
    And una Flecha con la condición topológica <topology>
    When el controlador emite el tick PlayMove
    Then AdvanceArrowUseCase reporta success = true con outcome = <outcome>
    And GameSession.consumeMove() se ejecuta exactamente 1 vez
    And movesRemaining queda en 4

    Examples:
      | topology                                       | outcome   |
      | ruta libre hacia una celda desocupada          | advanced  |
      | celda destino ocupada por otra flecha          | blocked   |
      | cabeza única apuntando a un puerto exit        | destroyed |
  ...
  Scenario: La última flecha sale del tablero y emite veredicto WON
    ...
    And movesRemaining queda en 3 (los movimientos sobrantes no afectan el veredicto de victoria)
  ...
  Scenario: Bloqueo persistente agota el presupuesto sin progreso topológico
    ...
    When el controlador emite ticks PlayMove consecutivos sobre F1 hasta agotar el presupuesto
    Then estado por tick:
      | tick | outcome | movesRemaining (después) | status      |
      | 1    | blocked | 1                        | IN_PROGRESS |
      | 2    | blocked | 0                        | LOST        |
    And el tablero conserva sus segmentos originales sin mutación
...
# ─────────────────────────────────────────────
  # DISTINCIÓN CRÍTICA SOBRE EL CONSUMO DE MOVIMIENTOS
  # Colisión entre flechas (outcome=blocked, success=true) → SÍ consume movimiento.
  #   Justificación: el jugador tomó una decisión cinemática válida que terminó en choque.
  # Error de sistema o input inválido (success=false)      → NO consume movimiento.
  #   Justificación: el presupuesto castiga decisiones del jugador, no bugs de infraestructura.
  # ─────────────────────────────────────────────